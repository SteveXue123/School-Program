from game import colors, actions, action, card
from basePlayer import basePlayer
import numpy as np
import random
from time import sleep

# BUG programmer和chemist类偶尔会出现在提示物不足时进行提示的bug
...
class businessman(gambler):
    def __init__(self):
        ...
        self.unknownCards = [(colors(c), n) for c in range(5) for n in [1, 1, 1, 2, 2, 3, 3, 4, 4, 5]] # deck + hand
        self.knowledge = [[{colors(a) for a in range(5)}, {b + 1 for b in range(5)}] for n in range(5)] # [[{:colors},{:number}]*5]
        self.player = 0
    ...
    def resolveTurn(self, index, playerAction, **kwargs):
        ...
        if playerAction.actionType == actions.discard and index == self.playerIndex:
            ...
            self.knowledge[playerAction.target] = [{colors(a) for a in range(5)}, {b + 1 for b in range(5)}]
...
class programmer(businessman):
    ...
    def otherwise(self):
        ...
        colorValue = [[1, 1, 1, 1, 1] for p in range(self.player - 1)]
